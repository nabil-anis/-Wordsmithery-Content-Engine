  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      <div 
        ...
        className={`w-full bg-white/5 border rounded-2xl px-6 py-4 flex items-center justify-between cursor-pointer transition-all duration-300 group ${
          isOpen ? 'border-purple-500/50 ring-1 ring-purple-500/20' : 'border-white/10 hover:border-white/20'
        }`}
      >
            ...
            {filteredOptions.length > 0 ? (
              <div className="py-2">
                {filteredOptions.map((opt) => (
                  <div 
                    key={opt}
                    className={`px-6 py-3 text-sm font-medium transition-all cursor-pointer flex items-center justify-between group/item ${
                      value === opt 
